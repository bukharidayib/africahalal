...
export default function QuotationsTab() {
  ...
  const handleSave = async (opts?: { downloadPdf?: boolean }) => {
      ...
      if (opts?.downloadPdf && savedId) {
        try {
          const { data, error } = await supabase.functions.invoke('generate-quotation-pdf', {
            body: { quotation_id: savedId },
          });
          if (error) throw error;
          const b64 = (data as any)?.pdf_base64;
          if (!b64) throw new Error('PDF not returned');
          const bin = atob(b64);
          ...
          a.href = url;
          a.download = `Quotation-${savedNumber || savedId}.pdf`;
