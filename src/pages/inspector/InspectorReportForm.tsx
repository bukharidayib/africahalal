...
// KPI_FIELDS moved to InspectorPerformance.tsx
...
export default function InspectorReportForm() {
  ...
  const handleEvidenceUpload = async (itemId: string, file: File) => {
    ...
    const { error } = await supabase.storage.from("Inspector-evidence").upload(path, file);
    if (error) {
    ...
    try {
      ...
      if (reportType === "daily_checklist") {
        ...
        if (itemsToInsert.length > 0) {
          const { error: itemsError } = await (supabase.from("Inspector_checklist_items" as any).insert(itemsToInsert as any) as any);
  ...
  if (isLoading) {
    return <InspectorLayout><div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div></InspectorLayout>;
  }
  ...
  return (
    <InspectorLayout>
      <div className="space-y-6 max-w-4xl">
        ...
        {/* Monthly Performance is now a separate module at /inspector/performance */}
