...
import { Mail, Lock, Loader2, User, Phone, Eye, EyeOff, Wand2, Search } from "lucide-react";
    ...
    try {
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: window.location.origin + '/inspector/signin',
        }
      });
      ...
      if (authData.user) {
        ...
        if (invitationToken) {
          const { error: acceptError } = await supabase.functions.invoke('accept-inspector-invitation', {
            body: { email, token: invitationToken },
          });
          if (acceptError) {
            console.warn("Could not accept inspector invitation:", acceptError.message);
          }
        }

        try {
          await supabase.functions.invoke('send-confirmation-email', {
            body: { email, full_name: fullName, portal: 'inspector' },
          });
        } catch (e) {
        ...
        toast({
          ...
          description: "Please verify your email address, then sign in to the Inspector Portal.",
        });
  ...
  return (
    <div className="flex min-h-screen bg-background">
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          <div className="mb-8">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-primary uppercase tracking-wider">Inspector Registration</span>
            </div>
            <h2 className="text-3xl font-bold tracking-tight text-foreground font-serif">Create Your Inspector Account</h2>
            <p className="mt-2 text-sm text-muted-foreground">Complete your profile to access the Inspector Portal</p>
          </div>

          {email && (
            <div className="bg-primary/5 border border-primary/20 rounded-lg px-4 py-3 mb-6">
              <p className="text-xs text-primary font-medium">🔍 Invited as Field Inspector</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="email" required value={email} className="pl-10 h-11 bg-muted/50" disabled readOnly />
              </div>
          ...
          <p className="text-center text-sm text-muted-foreground pt-6">
            ...
            <Link to="/inspector/signin" className="font-semibold text-primary hover:text-primary/80 underline-offset-4 hover:underline">
      ...
      <div className="hidden lg:relative lg:block lg:w-1/2 border-l overflow-hidden">
        <img className="absolute inset-0 h-full w-full object-cover" src="/auth-bg.png" alt="AHIS Inspector Portal" />
        <div className="absolute inset-0 bg-gradient-to-t from-primary via-primary/50 to-primary/20" />
        <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-white">
          <Search className="h-16 w-16 mb-6 opacity-90" />
          <h3 className="text-3xl font-bold font-serif leading-tight text-center mb-4">Inspector Portal</h3>
          <p className="text-lg text-white/80 leading-relaxed text-center max-w-md">
            Conduct on-site inspections, submit reports, and uphold Halal certification standards.
          </p>
        </div>
      </div>
    </div>
