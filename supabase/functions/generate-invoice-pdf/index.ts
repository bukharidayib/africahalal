...
async function buildInvoicePdf(invoiceId: string): Promise<Uint8Array> {
  ...
  const terms = `Payment is due by ${new Date(inv.due_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}. Pay via your AHI client portal (Billing > Pay Now). Please reference invoice #${inv.invoice_number} on all payments.`;
  let ty = 95;
